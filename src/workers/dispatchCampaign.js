...
import { buildSendJobId } from "./sendJobHelpers.js";

// Enqueue send jobs in bulk using Queue.addBulk(), with chunking and fallback.
export async function enqueueSendJobs(
  sendQueue,
  campaign_id,
  tenant_id,
  recipients,
) {
  const DISPATCH_CHUNK_SIZE = parseInt(
    process.env.CAMPAIGN_DISPATCH_CHUNK_SIZE || "500",
    10,
  );
  ...
  const REMOVE_ON_COMPLETE = process.env.REMOVE_ON_COMPLETE
    ? Number(process.env.REMOVE_ON_COMPLETE)
    : 1000;
  const REMOVE_ON_FAIL = process.env.REMOVE_ON_FAIL
    ? Number(process.env.REMOVE_ON_FAIL)
    : 5000;
  ...
  const jobs = recipients.map((r) => ({
    ...
    opts: {
      ...
      jobId: buildSendJobId(campaign_id, r.id),
    },
  }));
  ...
  for (const [idx, jobChunk] of chunks.entries()) {
    try {
      ...
      logger.info(
        `[DISPATCH] Enqueued chunk ${idx + 1}/${chunks.length} (${totalEnqueued}/${jobs.length})`,
      );
    } catch (err) {
      // addBulk failed (Redis/connection/other) — fall back to per-job with dedupe handling
      logger.warn(
        `[DISPATCH] addBulk failed for chunk ${idx + 1}: ${err.message}. Falling back to per-job enqueue.`,
      );
      for (const jobDef of jobChunk) {
        try {
          ...
          if (
            ...
            logger.debug(
              `[DISPATCH] Duplicate job ignored ${jobDef.opts.jobId}`,
            );
            continue;
          }
          totalFailed += 1;
          logger.warn(
            `[DISPATCH] Failed to enqueue jobId=${jobDef.opts.jobId}: ${e.message}`,
          );
        }
      }
      logger.info(
        `[DISPATCH] Chunk ${idx + 1} fallback completed: enqueued ${totalEnqueued} so far, failed ${totalFailed}`,
      );
    }
  }
  ...
  logger.info(
    `[DISPATCH] Bulk enqueue complete: enqueued=${totalEnqueued} failed=${totalFailed} durationMs=${duration}`,
  );
