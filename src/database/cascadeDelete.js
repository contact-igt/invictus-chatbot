...
export const cascadeDeleteTenant = async (tenant_id, transaction) => {
    ...
    try {
        ...
        await db.WhatsappTemplates.destroy({ where: { tenant_id }, transaction: t });
        await db.WhatsappCampaigns.destroy({ where: { tenant_id }, transaction: t });
        ...
        await db.AiAnalysisLog.destroy({ where: { tenant_id }, transaction: t });
        await db.TenantInvitations.destroy({ where: { tenant_id }, transaction: t });
...
export const cascadeDeleteDoctor = async (doctor_id, tenant_id, transaction) => {
    const t = transaction || await db.sequelize.transaction();
