    ...
    try {
        ...
        if (data.specializations && data.specializations.length > 0) {
            for (const specName of data.specializations) {
                ...
                if (!spec) {
                    throw new Error(`Specialization "${specName}" not found. Please create it first in the specializations master list.`);
                }
        ...
        if (data.availability && data.availability.length > 0) {
            for (const slot of data.availability) {
                const slots = slot.slots || [{ start_time: slot.start_time, end_time: slot.end_time }];
...
// ─── Get Doctor List ───
export const getDoctorListService = async (tenant_id) => {
    try {
     ...
     WHERE d.tenant_id = ? AND d.is_deleted = false
...
export const updateDoctorService = async (doctor_id, tenant_id, data) => {
    ...
    try {
       ...
       WHERE doctor_id = ? AND tenant_id = ? AND is_deleted = false`,
        ...
        if (data.country_code !== undefined) updateFields.country_code = data.country_code;
        if (data.mobile !== undefined) updateFields.mobile = data.mobile;
        if (data.email !== undefined) updateFields.email = data.email;
        if (data.status !== undefined) updateFields.status = data.status;
        if (data.consultation_duration !== undefined) updateFields.consultation_duration = data.consultation_duration;
        if (data.title !== undefined) updateFields.title = data.title;
        if (data.bio !== undefined) updateFields.bio = data.bio;
        if (data.profile_pic !== undefined) updateFields.profile_pic = data.profile_pic;
        if (data.experience_years !== undefined) updateFields.experience_years = data.experience_years;
        if (data.qualification !== undefined) updateFields.qualification = data.qualification;
...
export const softDeleteDoctorService = async (doctor_id, tenant_id) => {
    ...
    try {
        ...
        await transaction.commit();
        return { message: "Doctor and associated user account deleted successfully" };
    } catch (error) {
        await transaction.rollback();
        throw error;
    }
...
export const permanentDeleteDoctorService = async (doctor_id, tenant_id) => {
    try {
        const { cascadeDeleteDoctor } = await import("../../database/cascadeDelete.js");
...
export const restoreDoctorService = async (doctor_id, tenant_id) => {
    ...
    try {
        ...
        await transaction.commit();
        return { message: "Doctor and associated user account restored successfully" };
    } catch (error) {
        await transaction.rollback();
        throw error;
    }
