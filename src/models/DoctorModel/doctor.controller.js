...
export const createDoctorController = async (req, res) => {
    ...
    const { title, name, country_code, mobile, email, status,
        consultation_duration, specializations, availability,
        bio, profile_pic, experience_years, qualification } = req.body;
    ...
    if (consultation_duration !== undefined && consultation_duration !== null) {
        if (typeof consultation_duration !== 'number' || consultation_duration < 5 || consultation_duration > 240) {
            return res.status(400).send({
                message: "Consultation duration must be a number between 5 and 240 minutes"
            });
        }
    }
...
export const getDoctorListController = async (req, res) => {
    ...
    try {
        const result = await getDoctorListService(tenant_id);
