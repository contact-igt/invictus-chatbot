...
// ─── Create ───
export const createSpecializationService = async (tenant_id, name, description = null) => {
    try {
...
// ─── Get By ID ───
export const getSpecializationByIdService = async (specialization_id, tenant_id) => {
    try {
        return await db.Specializations.findOne({
...
// ─── Update ───
export const updateSpecializationService = async (specialization_id, tenant_id, data) => {
    try {
        const updateFields = {};
        ...
        const [updated] = await db.Specializations.update(
            updateFields,
            { where: { specialization_id, tenant_id, is_deleted: false } },
        );
...
export const toggleActiveStatusService = async (specialization_id, tenant_id) => {
    try {
        // Get current status
...
// ─── Soft Delete ───
export const deleteSpecializationService = async (specialization_id, tenant_id) => {
    try {
        ...
        const [usageRows] = await db.sequelize.query(
            `SELECT COUNT(*) as count FROM ${tableNames.DOCTOR_SPECIALIZATIONS}
...
export const restoreSpecializationService = async (specialization_id, tenant_id) => {
    try {
        const spec = await db.Specializations.findOne({
...
// ─── Permanent Delete ───
export const permanentDeleteSpecializationService = async (specialization_id, tenant_id) => {
    try {
