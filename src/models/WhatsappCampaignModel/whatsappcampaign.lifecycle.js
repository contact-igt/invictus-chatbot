...
export const getDeletedCampaigns = async (tenant_id, page = 1, limit = 20) => {
  ...
  const [rows] = await db.sequelize.query(
    `SELECT campaign_id, campaign_name, campaign_type, status,
            total_audience, delivered_count, read_count, replied_count,
...
// Controllers
export const softDeleteCampaignController = lifecycleHandler(
  async (req, res) => {
...
export const restoreCampaignController = lifecycleHandler(async (req, res) => {
  const data = await restoreCampaign(
...
export const hardDeleteCampaignController = lifecycleHandler(
  async (req, res) => {
    await hardDeleteCampaign(req.params.campaign_id, req.user.tenant_id);
    return res.status(200).json({
      message: "Campaign and all delivery records permanently deleted",
    });
  },
);

export const getDeletedCampaignsController = lifecycleHandler(
  async (req, res) => {
