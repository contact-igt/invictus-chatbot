...
console.log("[PAYMENT] Razorpay initialized with Key ID:", process.env.RAZORPAY_KEY_ID ? `${process.env.RAZORPAY_KEY_ID.slice(0, 8)}...` : "MISSING");
...
export const createRazorpayOrderService = async (tenant_id, amount) => {
  try {
    console.log(`[PAYMENT] Creating order for Tenant: ${tenant_id}, Amount: ${amount}`);
    ...
    if (error.error) console.error("[PAYMENT SERVICE] Razorpay API Error Details:", error.error);
    throw error;
  }
};
  ...
  if (expectedSignature === razorpay_signature) {
    ...
    try {
      ...
      io.to(`tenant-${tenant_id}`).emit("payment-update", {
        ...
        amount: amountInRupees,
        balance: (await db.Wallets.findOne({ where: { tenant_id } })).balance
      });
    } catch (err) {}
