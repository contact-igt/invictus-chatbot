...
export const getManagementService = async () => {
  ...
  const Query = `SELECT id , title, username , email ,  profile_picture , country_code, CONCAT(country_code,mobile) as mobile , role , status , last_login_at , created_at  FROM ${tableNames?.MANAGEMENT} ORDER BY created_at DESC `;
...
export const getManagementByIdService = async (user_id) => {
  const Query = `SELECT id , title, username , email ,  profile_picture , country_code, CONCAT(country_code,mobile) as mobile , role , status , last_login_at , created_at FROM ${tableNames?.MANAGEMENT} WHERE id = ? `;
