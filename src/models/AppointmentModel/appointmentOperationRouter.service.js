...
export const isAppointmentOperationDecisionEnabled = (
  ...
  if (DETERMINISTIC_SOURCES.has(decision.source)) return level >= ROUTER_MODES.deterministic;
...
const actionFromBookingReplyId = (replyId = "") => {
  ...
  if (baseId === "confirm_booking") return APPOINTMENT_OPERATION_ACTIONS.CONFIRM;
  if (baseId === "cancel_booking") return APPOINTMENT_OPERATION_ACTIONS.REJECT;
  if (baseId === "continue_appointment") return APPOINTMENT_OPERATION_ACTIONS.CONTINUE;
...
export const detectPreviousBotContextFromText = (message = "") => {
  ...
  if (/confirm your appointment booking|please confirm your appointment/.test(text)) {
  ...
  if (/select.*services|reason for visit|reason\/service|visit from the list/.test(text)) {
  ...
  if (/choose an available time|choose.*time slot|pick time|available time slots/.test(text)) {
    return PREVIOUS_BOT_CONTEXTS.ASKED_BOOKING_TIME;
  }
  if (/would you like to book|book a new appointment|book appointment/.test(text)) {
...
const routeFromPreviousBotContext = (context, message = "") => {
  ...
  if (
    ...
    return makeDecision({
      shouldHandle: true,
      route: APPOINTMENT_OPERATION_ROUTES.BOOK_APPOINTMENT,
      action: positive ? APPOINTMENT_OPERATION_ACTIONS.CONFIRM : APPOINTMENT_OPERATION_ACTIONS.BOOK,
      source: APPOINTMENT_OPERATION_SOURCES.PREVIOUS_BOT_CONTEXT,
      confidence: 0.88,
  ...
  if (context === PREVIOUS_BOT_CONTEXTS.ASKED_CONFIRM_CANCEL && (positive || negative)) {
    return makeDecision({
      shouldHandle: true,
      route: APPOINTMENT_OPERATION_ROUTES.MANAGE_APPOINTMENT,
      action: positive ? APPOINTMENT_OPERATION_ACTIONS.CONFIRM : APPOINTMENT_OPERATION_ACTIONS.REJECT,
      source: APPOINTMENT_OPERATION_SOURCES.PREVIOUS_BOT_CONTEXT,
      confidence: 0.9,
  ...
  if (context === PREVIOUS_BOT_CONTEXTS.ASKED_CONFIRM_RESCHEDULE && (positive || negative)) {
    return makeDecision({
      shouldHandle: true,
      route: APPOINTMENT_OPERATION_ROUTES.MANAGE_APPOINTMENT,
      action: positive ? APPOINTMENT_OPERATION_ACTIONS.CONFIRM : APPOINTMENT_OPERATION_ACTIONS.REJECT,
...
export const routeFromDeterministicPhrase = (message = "") => {
  ...
  const manageActionPatterns = [
    { action: APPOINTMENT_OPERATION_ACTIONS.EDIT, pattern: /\b(edit|change|modify|update)\b.*\b(appointment|booking)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.RESCHEDULE, pattern: /\b(reschedule|re schedule|re-schedule|move|postpone)\b.*\b(appointment|booking|it)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.RESCHEDULE, pattern: /\b(i want|i need|need|want|would like|please|can i|help me)\b.*\b(reschedule|re schedule|re-schedule|postpone|move)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.RESCHEDULE, pattern: /\b(change time|change doctor|shift it|shift appointment|move it)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.EDIT, pattern: /\b(i want|i need|need|want|would like|please|can i|help me)\b.*\b(edit|update|modify|change)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.CANCEL, pattern: /\b(cancel|delete)\b.*\b(appointment|booking)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.CANCEL, pattern: /\b(i want|i need|need|want|would like|please|can i|help me)\b.*\b(cancel|delete)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.CANCEL, pattern: /\b(not coming|can't come|cannot come|wont come|won't come)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.VIEW, pattern: /\b(show|view|see|check)\b.*\b(my\s+)?(appointment|appointments|booking|bookings)\b/ },
    { action: APPOINTMENT_OPERATION_ACTIONS.VIEW, pattern: /\b(my appointment|my appointments|my booking|my bookings|appointment details)\b/ },
  ];
  ...
  const bookingPattern =
    /^(book|appointment|book appointment|schedule appointment|need appointment|want appointment|doctor appointment|consultation booking|need doctor|meet doctor|create appointment)$/i;
  const bookingSentencePattern =
    /\b(book|schedule|create|need|want)\b.*\b(appointment|consultation|doctor|slot|booking)\b|\b(meet|see|visit)\b.*\b(doctor|dr\.?)\b/;
...
export const detectAppointmentIncomingFlowType = (
  normalizedMessage,
) => {
  const input = normalizeAppointmentOperationInput(normalizedMessage);
  const replyDecision = routeFromReplyId(input.buttonReplyId);
  if (replyDecision?.shouldHandle) return flowTypeFromRoute(replyDecision.route);

  const phraseDecision = routeFromDeterministicPhrase(input.messageText);
  if (phraseDecision?.shouldHandle) return flowTypeFromRoute(phraseDecision.route);
...
const normalizeAiHelperDecision = (raw = {}) => {
  ...
  const action = Object.values(APPOINTMENT_OPERATION_ACTIONS).includes(raw.action)
    ? raw.action
  ...
  return makeDecision({
    ...
    confidence,
    reason: normalizeText(raw.reason) || "AI helper classified appointment operation.",
...
export const getPreviousBotContext = async ({ tenantId, phone, contactId }) => {
  ...
  const previousBotMessage = [...chatHistory]
    .reverse()
    .find((entry) => entry.role === "assistant" && normalizeText(entry.content));
  return {
    previousBotContext: detectPreviousBotContextFromText(previousBotMessage?.content || ""),
    chatHistory,
  };
};
...
export const resolveAppointmentOperationDecision = ({
  ...
  if (activeManageSession && isManageEditInputState(activeManageSession.state)) {
    return makeDecision({
      shouldHandle: true,
      route: APPOINTMENT_OPERATION_ROUTES.MANAGE_APPOINTMENT,
      action: APPOINTMENT_OPERATION_ACTIONS.EDIT,
  ...
  if (classifierResult?.intent === "APPOINTMENT_ACTION") {
    return makeDecision({
      shouldHandle: true,
      route: APPOINTMENT_OPERATION_ROUTES.BOOK_APPOINTMENT,
      action: APPOINTMENT_OPERATION_ACTIONS.BOOK,
      ...
      confidence: 0.78,
      reason: "Classifier returned APPOINTMENT_ACTION.",
    });
  }

  if (classifierResult?.intent === "MANAGE_APPOINTMENTS_ACTION") {
    return makeDecision({
      shouldHandle: true,
      route: APPOINTMENT_OPERATION_ROUTES.MANAGE_APPOINTMENT,
      action: APPOINTMENT_OPERATION_ACTIONS.UNKNOWN,
      source: APPOINTMENT_OPERATION_SOURCES.CLASSIFIER_INTENT,
      confidence: 0.78,
      reason: "Classifier returned MANAGE_APPOINTMENTS_ACTION.",
    });
  }
...
export const routeAppointmentOperation = async ({
  ...
  const validActiveManageSession =
    activeManageSession && !isManageAppointmentSessionExpired(activeManageSession)
      ? activeManageSession
      : null;
  ...
  if (!decision.shouldHandle && !skipAiHelper && looksAppointmentLike(input.messageText)) {
    decision = await runAiHelper({
      ...
      return (
        ...
        makeDecision({
          ...
          source: APPOINTMENT_OPERATION_SOURCES.AI_HELPER,
          confidence: 0.55,
          reason: "AI helper failed; appointment-like text routed to safe manage lookup.",
        })
      );
    });
  }
...
export const logAppointmentOperationRouterDecision = ({
  ...
  console.log("[APPOINTMENT_OPERATION_ROUTER]", JSON.stringify(payload));
};
