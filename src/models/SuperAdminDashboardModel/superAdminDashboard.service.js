...
export const getSuperAdminDashboardStatsService = async (period = "30days") => {
    ...
    const periodWhere = periodStart ? { created_at: { [Op.gte]: periodStart } } : {};
    const prevPeriodWhere = periodStartPrev && periodStart
    ...
    // ─── 2. Management Users ─────────────────────────────────────────
    const [totalAdmins, activePlatformAdmins, superAdminCount] = await Promise.all([
        db.Management.count({ where: { is_deleted: false } }),
        db.Management.count({ where: { role: "platform_admin", status: "active", is_deleted: false } }),
        db.Management.count({ where: { role: "super_admin", is_deleted: false } }),
    ]);
    ...
    const [
        ...
        periodStartPrev
            ...
            : Promise.resolve(0),
        db.Leads.count({ where: { is_deleted: false, created_at: { [Op.gte]: todayAtStart } } }),
    ]);
    ...
    const [connectedAccounts, disconnectedAccounts] = await Promise.all([
        db.Whatsappaccount.count({ where: { status: "active", is_deleted: false } }),
    ...
    const recentTenants = await db.Tenants.findAll({
        where: { is_deleted: false },
        attributes: ["tenant_id", "company_name", "owner_name", "owner_email", "status", "subscription_plan", "created_at"],
    ...
    const [totalAppointments, appointmentsToday] = await Promise.all([
        ...
        db.Appointments.count({ where: { created_at: { [Op.gte]: todayAtStart } } }),
    ]);
    ...
    // ─── 14. AI Analysis Stats ───────────────────────────────────────
    const [totalAiLogs, aiLogsByType] = await Promise.all([
        db.AiAnalysisLog.count({ where: { ...periodWhere } }),
    ...
    const expiringTenants = await db.Tenants.findAll({
        ...
        attributes: ["tenant_id", "company_name", "subscription_end_date", "status"],
    ...
    const escalationsToday = await db.LiveChat.count({
        where: { status: "active", assigned_admin_id: { [Op.ne]: null }, created_at: { [Op.gte]: todayAtStart } },
    });
    ...
    // ─── 23. Critical Alerts ─────────────────────────────────────────
    const [expiredSubsCount, lowWalletCount, disconnectedWACount] = await Promise.all([
        db.Tenants.count({ where: { is_deleted: false, status: "expired" } }),
        db.Wallets.count({ where: { balance: { [Op.lt]: 100 } } }),
        db.Whatsappaccount.count({ where: { is_deleted: false, status: { [Op.notIn]: ["active", "verified"] } } }),
    ]);
    ...
    recentExpired.forEach((t) => {
        recentPlatformEvents.push({
            event: t.status === "expired" ? "Subscription Expired" : "Organization Suspended",
            detail: `${t.company_name} — ${t.status}`,
            time: t.updated_at,
            type: t.status === "expired" ? "subscription_expired" : "tenant_suspended",
            severity: "critical",
        });
    });
    ...
    const recentDisconnected = await db.Whatsappaccount.findAll({
        where: { is_deleted: false, status: { [Op.notIn]: ["active", "verified"] } },
    ...
    const disconnTenantDetails = disconnTenantIds.length > 0
        ? await db.Tenants.findAll({
        ...
        : [];
    const disconnMap = {};
    disconnTenantDetails.forEach((t) => { disconnMap[t.tenant_id] = t; });
    ...
    return {
        ...
        tenantUsers: {
            ...
            byRole: tenantUsersByRole.map((r) => ({ role: r.role, count: parseInt(r.count, 10) })),
        },
        messages: {
        ...
        campaigns: {
            ...
            byStatus: campaignsByStatus.map((c) => ({ status: c.status, count: parseInt(c.count, 10) })),
        },
        whatsappAccounts: {
        ...
        aiAnalysis: {
            total: totalAiLogs,
            byType: aiLogsByType.map((l) => ({ type: l.type, count: parseInt(l.count, 10) })),
        },
