 ...
 */
export const uploadMediaToMetaForTemplate = async (
  tenant_id,
  mediaUrl,
  format,
  options = {},
  ...
  try {
    ...
    if (!whatsappAccount || whatsappAccount.status !== "active") {
      throw new Error(
        "No active WhatsApp account found to perform media upload.",
      );
    }
    ...
    // Safety overrides
    if (format === "VIDEO" && fileType !== "video/mp4") {
      fileType = "video/mp4";
    }
    if (format === "IMAGE" && !["image/jpeg", "image/png"].includes(fileType)) {
      fileType = "image/jpeg";
    }
    if (format === "DOCUMENT" && fileType !== "application/pdf") {
      fileType = "application/pdf";
    }
    ...
    if (!appId) {
      throw new Error(
        "Failed to extract App ID from access token. Cannot proceed with Resumable Upload.",
      );
    }
    ...
    if (error.response) {
      ...
      throw new Error(
        `Meta Resumable Upload Failed: ${error.response.data?.error?.message || error.message}`,
      );
    }
    throw error;
  }
