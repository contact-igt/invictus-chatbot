 ...
 */
export const renderTemplateContent = async (template_id, dynamicComponents = []) => {
  try {
    ...
    if (header) {
      if (header.header_format === 'text' && header.text_content) {
        ...
        const headerParams = dynamicComponents?.find((c) => c.type === "header")?.parameters || [];
        ...
        if (headerComp?.parameters?.[0]) {
          ...
          mediaUrl = p.image?.link || p.video?.link || p.document?.link || p.file?.link || "";
        }
    ...
    if (body && body.text_content) {
      ...
      const bodyParams = dynamicComponents?.find((c) => c.type === "body")?.parameters || [];
