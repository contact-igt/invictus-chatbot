 ...
 */
export const processKnowledgeWithAi = async (text, prompt) => {
  if (!text) return text;

  // Use a sensible default prompt if none is provided to ensure clean data
  const finalPrompt = prompt?.trim() || "Extract the main readable content from this website. Remove all boilerplate like headers, footers, and scripts. If you see raw HTML, parse it to find the actual article or core information. Format the output clearly with headings.";
