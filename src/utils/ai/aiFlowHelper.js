 ...
 */
export const buildAiSystemPrompt = async (tenant_id, contact_id, languageInfo, userMessage) => {
  const now = new Date(
  ...
  const hospitalPrompt = (await getActivePromptService(tenant_id)) || DEFAULT_SYSTEM_PROMPT;
  const commonBasePrompt = getCommonBasePrompt(languageInfo);
  
  const { chunks, resolvedLogs, sources } = await searchKnowledgeChunks(tenant_id, userMessage);
  
  const knowledgeContext = chunks && chunks.length > 0 ? chunks.join("\n\n") : "No relevant uploaded documents.";
  const resolvedContext = resolvedLogs && resolvedLogs.length > 0 ? resolvedLogs.join("\n\n") : "";
  ...
  try {
    ...
    if (contact_id) {
      ...
      if (recentAppts && recentAppts.length > 0) {
        ...
        const futureAppts = recentAppts.filter(a => !a.is_deleted && a.status !== "Cancelled" && a.appointment_date >= todayStr);
        const pastAppts = recentAppts.filter(a => !a.is_deleted && a.status !== "Cancelled" && a.appointment_date < todayStr);
